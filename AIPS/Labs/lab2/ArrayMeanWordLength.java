...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        for (int i = 0; i < arr.getSize(); i++) {
            counter += arr.get(i).length();
        }
        float average = (float) counter / arr.getSize();
        float minDiff = arr.get(0).length() - average;
//        System.out.println("the average is: " + average);
        for (int i = 0; i < arr.getSize(); i++) {
            if (arr.get(i).length() == average) {
                return arr.get(index);
            } else if (arr.get(i).length() > arr.get(index).length() && (arr.get(i).length() < average + 1 || arr.get(i).length() < average - 1)) {
                index=i;
            }
        }
        return arr.get(index);

...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        int strongestCard = firstSorcererCards.getFirst().element.getHealth() * firstSorcererCards.getFirst().element.getMagicPower();
        int indexStrongest = 0;
        for (int i = 1; i < firstSorcererCards.size(); i++) {
            ...
            if (temp.element.getHealth() * temp.element.getMagicPower() > strongestCard) {
                strongestCard = temp.element.getHealth() * temp.element.getMagicPower();
                indexStrongest = i;
            }
        }
        if (indexStrongest != 0) {
            SLLNode<Card> temp = firstSorcererCards.getFirst();
            int counter = 0;
            while (temp != null && counter < indexStrongest) {
                temp = temp.succ;
                counter++;
            }
            ...
            counter = 0;
            while (temp2 != null && counter < secondSorcererCards.size() / 2) {
                temp2 = temp2.succ;
                counter++;
            }
            secondSorcererCards.insertBefore(temp.element, temp2);
        } else {
            firstSorcererCards.deleteFirst();
            secondSorcererCards.insertLast(firstSorcererCards.getFirst().element);
        }
    }
